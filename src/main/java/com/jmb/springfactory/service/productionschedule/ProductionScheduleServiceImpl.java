...
public class ProductionScheduleServiceImpl
    ...
    @Override
    public List<ProductionScheduleDto> findAll(final QueryProductionScheduleObject queryObject) {

        final Stream<ProductionSchedule> scheduleList;
                
        if (queryObject.isEmpty()) {
            serviceLog.info("Retrieve all schedules");
            scheduleList = productionScheduleMySQLService.findAll();
        } else {
            serviceLog.info("Search by parameters");
            scheduleList = findAllByQueryObject(queryObject); 
        }

        return scheduleList.map(this::entityToDto).collect(Collectors.toList());
                
    }
    
    private Stream<ProductionSchedule> findAllByQueryObject(final QueryProductionScheduleObject queryObject) {
        val scheduleQuery = getMapper().map(queryObject, getClazz());
        return productionScheduleMySQLService.findAllByExample(scheduleQuery);
    }
